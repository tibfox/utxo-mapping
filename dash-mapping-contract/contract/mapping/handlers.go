...
//
// Audit ALLOW (CVSS 7.5, port of BTC review7 MED-1 / D13): granting an
// allowance hands spend authority over the owner's tokens to the spender,
// so it requires ACTIVE auth — the same gate as transfer/unmap. Without
// it a posting-key-only call could approve a spender and then drain the
// balance via transferFrom. Mirror BTC's HandleApprove at
// btc-mapping-contract/contract/mapping/handlers.go:266-272.
...
//
// Audit ALLOW: allowance changes require active auth (see HandleApprove).
